...
void loop() {
    ...
    recive_from_arduino=Serial.readStringUntil('\n');
    
    if (recive_from_arduino.indexOf("g_act") != -1){
    ...
    /*--------------------------------------------------------------------------*/
     ...
     if (recive_from_arduino.indexOf("s_sen") != -1){

   
    
         url_sensor="http://10.10.10.10/tools/get_config_sensor.php?d=f899139df5e1059396431415e770c6ddp";
         url_sensor+="&p1=0";
         url_sensor+="p2=0";
         url_sensor+="p3=0";
         url_sensor+="p4=0";
    ...
    if((WiFiMulti.run() == WL_CONNECTED)) {
        ...
        /////Serial.print("[HTTP-SENSOR] begin...\n");
        // configure traged server and url

        http.begin(url_sensor); //HTTP md5 + PortStatus from Arduino send

        /////Serial.print("[HTTP-SENSOR] GET...\n");
        // start connection and send HTTP header
        int httpCode = http.GET();
        ...
        if(httpCode > 0) {
            // HTTP header has been send and Server response header has been handled
            /////Serial.printf("[HTTP-SENSOR] GET... code: %d\n", httpCode); // Ver contenido http-request
            ...
            /////Serial.printf("[HTTP-SENSOR] GET... failed, error: %s\n", http.errorToString(httpCode).c_str());
        }
